...
public class SelectChungMyungScreen extends JPanel {
    ...
    public SelectChungMyungScreen(GameGUI gui) {
        ...
        next = createButton();
        next.addActionListener(e -> { SoundManager.playSFX(SoundManager.SFX_BUTTON); gui.showScreen("SelectKennethScreen"); });
        this.add(next);

        prev = createButton();
        prev.addActionListener(e -> { SoundManager.playSFX(SoundManager.SFX_BUTTON); gui.showScreen("SelectBrivanScreen"); });
        this.add(prev);

        info = createButton();
        info.addActionListener(e -> { SoundManager.playSFX(SoundManager.SFX_BUTTON); gui.showScreen("ChungInfoScreen"); });
        this.add(info);
